...
\begin{tikzpicture}[remember picture, overlay]
    % ---- Background panel ----
    \fill[soft] (current page.north west) rectangle ([yshift=-7cm]current page.north east);
    \fill[primary]   (current page.south west) rectangle ([yshift=2.5cm]current page.south east);

    % ---- Top accent stripe ----
    \fill[primary] (current page.north west) rectangle ([yshift=-0.3cm]current page.north east);
    \fill[accent!80!primary] ([yshift=-0.3cm]current page.north west) rectangle ([yshift=-0.45cm]current page.north east);

    % ---- Bottom accent stripe (mirrored) ----
    \fill[primary] (current page.south west) rectangle ([yshift=0.45cm]current page.south east);
    \fill[accent!80!primary] ([yshift=0.3cm]current page.south west) rectangle ([yshift=0.45cm]current page.south east);

    % ---- Decorative left sidebar ----
    \fill[primary] ([xshift=0cm, yshift=-7cm]current page.north west) rectangle ([xshift=0.35cm, yshift=2.5cm]current page.north west);

    % ---- Decorative right accent: dotted vertical strip ----
    \foreach \y in {1.5,1.8,...,6.5} {
        \fill[primary!40] ([xshift=-1.4cm, yshift=-\y cm]current page.north east) circle (0.08);
    }
    \foreach \y in {1.65,1.95,...,6.65} {
        \fill[accent!70] ([xshift=-1.7cm, yshift=-\y cm]current page.north east) circle (0.05);
    }
\end{tikzpicture}

% ----- Top banner content (institution, faculty) ----
\vspace*{0.4cm}
\begin{center}
    {\color{primary}\rule{\linewidth}{0.4pt}} \\[0.35cm]
    {\large\bfseries\color{primary} DEPARTMENT OF COMPUTER SCIENCE \\ AND ENGINEERING} \\[0.25cm]
    {\large\itshape\color{primary!75!black} Faculty of Science and Information Technology} \\[0.35cm]
...
% ----- Course banner ----
\begin{center}
    {\Large\bfseries\color{primary!90!black} Artificial Intelligence \\ and Machine Learning Lab} \\[0.25cm]
    {\large\itshape\color{primary!75!black} Course Code: CSE -- 3-2 \textbullet{} Laboratory Report}
\end{center}
...
\begin{center}
    \begin{minipage}{0.92\linewidth}
        ...
        \begin{tcolorbox}[
            ...
            enhanced, drop shadow={primary!60!black, opacity=0.25}
        ]
            {\Huge\bfseries\color{primary} A Comparative Study} \\[0.25cm]
            {\Huge\bfseries\color{primary} of Six Foundational} \\[0.25cm]
            {\Huge\bfseries\color{primary} Machine Learning Models} \\[0.5cm]
            \begin{tikzpicture}
                \draw[accent, line width=1.2pt] (0,0) -- (8,0);
                \node[circle, fill=accent, draw=primary, line width=0.8pt,
                      minimum size=0.55cm, inner sep=0pt] at (4,0) {\color{white}\scriptsize\bfseries 6};
            \end{tikzpicture} \\[0.4cm]
            {\Large\color{primary!75!black} Linear \& Logistic Regression $\bullet$ SVM $\bullet$ KNN \\[2pt]
             Decision Trees (CART vs ID3) $\bullet$ K-Means Clustering}
        \end{tcolorbox}
    \end{minipage}
\end{center}
...
% ----- Author / submission info table ----
\begin{center}
\renewcommand{\arraystretch}{1.55}
\setlength{\arrayrulewidth}{0.5pt}
\arrayrulecolor{primary}
\begin{tabularx}{0.88\linewidth}{>{\bfseries\color{primary}}l X}
    \toprule
    \rowcolor{soft}\multicolumn{2}{l}{\bfseries\color{primary}\large Submitted by} \\[4pt]
    \midrule
    Name                              & Ahsanul Haque \\
    Student ID                        & 220119 \\
    Section                           & 3 -- 2 \\
    \midrule
    \rowcolor{soft}\multicolumn{2}{l}{\bfseries\color{primary}\large Submitted to} \\[4pt]
    \midrule
    Course Instructor                 & Sabbir Ahmed \\
    Course Title                      & AI \& Machine Learning Lab \\
    \midrule
    \rowcolor{soft}\multicolumn{2}{l}{\bfseries\color{primary}\large Submission} \\[4pt]
    \midrule
    Date of Submission                & June 15, 2026 \\
    Report Type                       & Comprehensive Lab Report \\
    \bottomrule
\end{tabularx}
\end{center}

% ----- Footer (in bottom blue band) ----
\vfill
\begin{tikzpicture}[remember picture, overlay]
    \node[anchor=center, text=white] at ([yshift=1.25cm]current page.south) {%
        \begin{minipage}{0.92\linewidth}
            \centering
            {\bfseries\large Source Code \& Datasets} \\[2pt]
            \texttt{\small https://github.com/ahsanjust/Artificial-Intelligence-and-Machine-Learning-Lab}
        \end{minipage}
    };
\end{tikzpicture}
\restoregeometry
